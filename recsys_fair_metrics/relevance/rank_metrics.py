...
class RelevanceRank(object):
    ...
    def fit(self, df: pd.DataFrame, item_column: str, rec_list: str) -> pd.DataFrame:
        ...
        with Pool(os.cpu_count()) as p:
            ...
            df["relevance_list"] = list(
                tqdm(
                    p.starmap(
                        ...
                        zip(df[rec_list], df[item_column]),
                    ),
                    total=len(df),
                )
            )
            ...
            for m in self._metrics:
                ...
                df[m] = list(
                    tqdm(
                        p.map(
                            functools.partial(self._func_metric[metric], k=int(k)),
                            df["relevance_list"],
                        ),
                        total=len(df),
                    )
        ...
        data.append(
            go.Bar(
                ...
                error_y=dict(
                    type="data",
                    array=[
                        mean_confidence_interval(self._df_metrics[m])[1]
